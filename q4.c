...
/*
    ...
    A forma de divisão de trabalho entre theads foi escolhina de tal forma:
    Se o mapa tem tamanho x e y, x será dividido pelo número de theads n de tal forma
...
void MesmaIlha(int xMin,int xFin,int yMin,int yFin,int x,int y)
{
    ...
    m1[x][y]=2;
    m2[x][y]=auxMesmaIlha; //modificando a matriz auxiliar que será necessaria para juntar o mapa
...
{
    //Backtrackin auxiliar, apenas para resolver as regiões de fronteira das threads
...
void* Partes(void *ptr)                         //Processamento de cada thread
{
    ...
    for(int i=xMin;i<xMax;i++)
    {
        for(int j=0;j<Ytemp;j++)
        {
            if(m1[i][j]==1)
            {
                pthread_mutex_lock(&mymutex);       //mutex necessário devido a condição de corrida 
                num_ilhas = num_ilhas + 1; 
                MesmaIlha(xMin,xMax,0,Ytemp,i,j);   //Backtracking
                auxMesmaIlha++;                     //Linha que gera condição de corrida, cada ilha deve ter um valor diferente
...
void definirEntrada(int n, int x,int y)//Aqui será definida a area dde atuação de cada thread
{
    int z=x;
    int m=x/n;
    int i=0;
    ...
    saux[i].Ytemp=y;
    return;
    //xMin e Xmax irão variar a cada theads
    //o valor de y sempre terá o mesmo alcançe
}
...
int main()
{
    ...
    definirEntrada(n,x,y);                                      //Aqui será realizada a divisão de trabalho das theads
    for(int i=0;i<n;i++)
    {
        ...
        //Inserindo os valores que serão utilizados por cada thead
    }
    for(int i=0;i<x;i++)
    {
    ...
    for(int i=0;i<n;i++)
    {
        int ret = pthread_create( &thread[i], NULL, Partes, (void*)&Entrada[i]); //criação  das theads, o vetor Entrada determina em que parte do mapa elas irão agir
    }
    for(int i=0;i<n;i++)
    {
        pthread_join(thread[i], NULL); //só continuar o codigo após as theads serem executadas
    }

    
    for(int i=0;i<x;i++) //Nessa parte do código são resolvidas as regiões de fronteira no mapa(entre duas theads)
    {
        for(int j=0;j<y;j++)
        {
            if(m2[i][j]!=0)
